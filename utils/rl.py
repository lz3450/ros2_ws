...
def generate_dot_file(node_roles: dict[str, dict[str, list]], output_file: str):
    ...
    for medium in list(media_nodes.keys()):
        assert len(media_nodes[medium]) <= 2, f"Medium '{medium}' has multiple roles: {media_nodes[medium].keys()}"
        if len(media_nodes[medium]) == 1:
            # print(f"Warning: Medium '{medium}' has only one role: {media_nodes[medium].keys()}")
            del media_nodes[medium]
